...
if (isset($_GET['txtID'])) {
    ...
    $sentencia = $conexion->prepare("SELECT * FROM `cotillon` WHERE id = :id");
...
if ($_POST) {
    ...
    $sentencia = $conexion->prepare("UPDATE `cotillon` 
    SET `producto`=:producto, `precio`=:precio, `stock`=:stock, `fecha de ingreso`=:fechadeingreso 
    WHERE id=:id");
